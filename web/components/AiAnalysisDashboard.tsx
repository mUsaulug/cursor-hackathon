...
export default function AiAnalysisDashboard({ role }: AiAnalysisDashboardProps) {
  ...
  const fetchSummary = useCallback(async () => {
    const res = await apiFetch("/api/v1/vision/summary", role, {
      cache: "no-store",
    });
    if (!res.ok) {
  ...
  const runAnalyze = useCallback(
    async (url: string, previewSrc: string) => {
      ...
      try {
        const res = await apiFetch(url.replace(/^https?:\/\/[^/]+/, ""), role, {
          method: "POST",
        });
        if (!res.ok) {
  ...
  const handleDemo = useCallback(
    (scene: DemoScene) => {
      ...
      void runAnalyze(
        `/api/v1/vision/analyze?${params.toString()}`,
        scene.imagePath,
      );
    },
    [runAnalyze],
  ...
  const handleUpload = useCallback(
    (file: File) => {
      ...
      void (async () => {
        ...
        try {
          const res = await apiFetch("/api/v1/vision/analyze", role, {
            method: "POST",
            body: form,
          });
          if (!res.ok) {
    ...
    async (detectionId: string, decision: "accepted" | "rejected") => {
      ...
      try {
        const res = await apiFetch(
          `/api/v1/vision/reviews/${detectionId}`,
          role,
          {
            method: "PATCH",
